...
import { useRoVignetteConfirmation } from '../hooks/useRoVignetteConfirmation'
import { useRoVignettePreOrder } from '../hooks/useRoVignettePreOrder'
import { RoVignetteContactFormValues, RoVignetteErrorType } from '../types'
...
export const RoVignetteContact = ({ price }): JSX.Element => {
  const { translate, locale } = useTranslation()
  const { basePath, push, query } = useRouter()

  const { confirm } = useRoVignetteConfirmation()
  const { updateContacts } = useRoVignettePreOrder()
  ...
  const onSubmit = async () => {
    ...
    const updated = await updateContacts(
      getValues() as RoVignetteContactFormValues,
      query?.order as any
    )
    if (!updated) {
      setError('form', { type: RoVignetteErrorType.confirmFailed })
      setIsLoading(false)
      return
    }
    ...
    ga.addToCart(Number(price) / 100, orderId, ItemName[InsuranceType.RO_VIGNETTE], 'iAsig')
    fbq.addToCart(Number(price) / 100, orderId, ItemName[InsuranceType.RO_VIGNETTE], 'iAsig')
    fba.addToCart(Number(price) / 100, orderId, ItemName[InsuranceType.RO_VIGNETTE], 'iAsig')
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-16 lg:pb-24">
      ...
      {errors?.form?.type === RoVignetteErrorType.confirmFailed && (
