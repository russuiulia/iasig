...
import { useMdVignetteConfirmation } from '../hooks/useMdVignetteConfirmation'
import { useMdVignettePreOrder } from '../hooks/useMdVignettePreOrder'
import { MdVignetteContactFormValues, MdVignetteErrorType } from '../types'
...
export const MdVignetteContact = ({ price }): JSX.Element => {
  const { translate, locale } = useTranslation()
  const { basePath, push, query } = useRouter()

  const { confirm } = useMdVignetteConfirmation()
  const { updateContacts } = useMdVignettePreOrder()
  ...
  const onSubmit = async () => {
    ...
    const updated = await updateContacts(
      getValues() as MdVignetteContactFormValues,
      query?.order as any
    )
    if (!updated) {
      setError('form', { type: MdVignetteErrorType.confirmFailed })
      setIsLoading(false)
      return
    }
    ...
    ga.addToCart(Number(price) / 100, orderId, ItemName[InsuranceType.MD_VIGNETTE], 'iAsig')
    fbq.addToCart(Number(price) / 100, orderId, ItemName[InsuranceType.MD_VIGNETTE], 'iAsig')
    fba.addToCart(Number(price) / 100, orderId, ItemName[InsuranceType.MD_VIGNETTE], 'iAsig')
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-16 lg:pb-24">
      ...
      {errors?.form?.type === MdVignetteErrorType.confirmFailed && (
