...
import { useRoadAssistanceEUConfirmation, useRoadAssistanceEUPreOrder } from '../hooks'
import { RoadAssistanceEUContactFormValues, RoadAssistanceEUErrorType } from '../types'
...
export const RoadAssistanceEUContact = ({ price }): JSX.Element => {
  const { translate, locale } = useTranslation()
  const { basePath, push, query } = useRouter()

  const { confirm } = useRoadAssistanceEUConfirmation()
  const { updateContacts } = useRoadAssistanceEUPreOrder()
  ...
  const onSubmit = async () => {
    ...
    const updated = await updateContacts(
      getValues() as RoadAssistanceEUContactFormValues,
      query?.order as any
    )
    if (!updated) {
      setError('form', { type: RoadAssistanceEUErrorType.confirmFailed })
      setIsLoading(false)
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-16 lg:pb-24">
      ...
      {errors?.form?.type === RoadAssistanceEUErrorType.confirmFailed && (
